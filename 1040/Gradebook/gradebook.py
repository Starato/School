...
def getAssgmtAvg():
    ...
    for y in range(len(gradebook[0])):
        sum = 0
        for x in range(len(gradebook)):
            sum += gradebook[x][y]  
        aAvg.append('{0:.2f}'.format(sum/length))
    return aAvg

def getNumAssgmt():
    avg = getAssgmtAvg()
    print('Assignment Averages:')
    for x, y in zip(range(len(gradebook[0])), avg):
        print('Assignment ' + str(x+1) + ': ' + str(y))

def getStuAvg():
    sAvg = []
    length = len(gradebook[0])
    for x in range(len(gradebook)):
        sum = 0
        for y in range(len(gradebook[0])):
            sum += gradebook[x][y]  
        sAvg.append('{0:.2f}'.format(sum/length))
    return sAvg

def getNumStu():
    avg = getStuAvg()
    print('\n' + 'Student Averages:')
    for x, y in zip(range(len(gradebook)), avg):
        print('Student ' + str(x+1) + ': ' + str(y))
